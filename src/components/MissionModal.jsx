      ...
      <div className="bg-neutral-800 border border-neutral-600 rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col transform transition-all">
        ...
        <div className="p-6 flex flex-col gap-4">
           <div className="bg-black/40 p-3 rounded border border-neutral-700">
             <span className="text-xs text-neutral-500 block uppercase tracking-wider mb-1">Selected Route</span>
             <strong className="text-white text-xl">{activeMission.name}</strong>
           </div>

           <div className="grid grid-cols-2 gap-3">
             <div className="bg-black/40 p-3 rounded border border-neutral-700">
               <span className="text-xs text-neutral-500 block uppercase tracking-wider mb-1">Max Altitude</span>
             ...
             <div className="bg-black/40 p-3 rounded border border-neutral-700">
               <span className="text-xs text-neutral-500 block uppercase tracking-wider mb-1">Waypoints</span>
               <strong className="text-cyan-400 text-2xl">{activeMission.details?.total_waypoints || 0}</strong>
             </div>
           </div>

           <div className="bg-black/40 p-3 rounded border border-neutral-700">
              <span className="text-xs text-neutral-500 block uppercase tracking-wider mb-1">Operational Flight Profile</span>
              <strong className="text-white text-lg">{activeMission.details?.flight_type || "Unknown"}</strong>
           </div>

           <div className="bg-black/40 p-3 rounded border border-neutral-700">
              <span className="text-xs text-neutral-500 block uppercase tracking-wider mb-1">Planning Platform</span>
        ...
        <div className="px-6 pb-6">
          ...
          {missionPhase === 'AWAITING_APPROVAL' && (
            <div className="bg-black/60 p-2 rounded border border-neutral-600 text-xs mt-1">
              <div className="text-neutral-400 mb-2 tracking-widest uppercase text-[10px]">Authorization Status:</div>
              <div className="flex justify-between items-center mb-1">
                <span>Pilot (Telegram):</span>
                <span className={approvals.pilot ? "text-green-500 font-bold" : "text-yellow-500 animate-pulse"}>
                  {approvals.pilot ? "✔ OK" : "PENDING..."}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span>Room Manager:</span>
                {userRole === 'responsabile' && !approvals.manager ? (
                  <button onClick={onManagerApprove} className="bg-blue-600 hover:bg-blue-500 text-white px-2 py-0.5 rounded text-[10px]">Approve</button>
                ) : (
                  <span className={approvals.manager ? "text-green-500 font-bold" : "text-yellow-500 animate-pulse"}>
                    {approvals.manager ? "✔ OK" : "PENDING..."}
                  </span>
                )}
              </div>
            </div>
